"""
Sample code automatically generated on 2023-05-17 13:51:22
...
class GenoNLP:
    def __init__(self, Kold, beta, gamma, delta, c, y, eigenvaluesOld, eigenvectors, np):
        ...
        assert self.eigenvectors_rows == self.y_rows == self.Kold_rows == self.Kold_cols == self.alphas_rows
        assert self.eigenvectors_cols == self.eigenvalues_rows
...
def solve(Kold, beta, gamma, delta, c, y, eigenvaluesOld, eigenvectors, np):
    start = timer()
...
def learn_spectral_properties(eigenvalues_old, eigenvectors, label, hyperparameters):
    ...
    c_param = hyperparameters['c_param']

    result, alphas, eigenvalues_new = solve(K_old, beta, gamma, delta, c_param, y, eigenvalues_old, eigenvectors,
                                            np=numpy)

    return eigenvalues_new, alphas
