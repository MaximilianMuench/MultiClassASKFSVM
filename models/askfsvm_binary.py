...
class ASKFSVMBinary(BaseEstimator, ClassifierMixin):
    def __init__(self, C=1.0, max_iter=1000, subsample_size=1.0, beta=1, gamma=1, delta=1):
    ...
    def fit(self, Ks, y):
        ...
        # result, self.alphas, self.new_eigenvalues = ASFSolver.solve(Kold=K_old, beta=self.beta, gamma=self.gamma,
        #                                                             delta=self.gamma, c=self.C, y=y,
        #                                                             eigenvaluesOld=self.old_eigenvalues,
        #                                                             eigenvectors=self.eigenvectors, np=np)
